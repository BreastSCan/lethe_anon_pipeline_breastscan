...
def _anonymization_info(anon_script: str):
    ...
    root = tree.getroot()
    params = [(e.attrib["t"], e.text) for e in root.findall("p")]
    es = [
        (e.attrib["n"], e.text)
...
def hash_patient_id(patient_id: str, *, secret_key: str, prefix: str) -> str:
    """
...
def _parse_and_hash_csv(
    ...
    Returns:
        None
    """
    logger.info(f"Parsing and hashing CSV file {input_file.name}")
    with (
        ...
        if verbose:
            logger.debug(f"Detected CSV dialect: {dialect.to_dict()}")
        fp.seek(0)
        ...
        # I opted for RFC4180 to ensure consistency (the Robustness principle).
        ...
        logger.info(
            f"Wrote {len(rows)} rows to {output_file.name} in RFC4180 CSV format"
        )
